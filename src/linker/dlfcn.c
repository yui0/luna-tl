...
void *bionic_dlsym(void *handle, const char *symbol)
{
    ...
    if(likely(sym != 0)) {
        bind = AELF(ST_BIND)(sym->st_info);

        if(likely((bind == STB_GLOBAL) && (sym->st_shndx != 0))) {
            uintptr_t ret = sym->st_value + found->base;
            pthread_mutex_unlock(&apkenv_dl_lock);
...
int bionic_dladdr(const void *addr, Dl_info *info)
{
    ...
    if(si) {
        ...
        AElf(Sym) *sym = apkenv_find_containing_symbol(addr, si);
...
#else
#error Unsupported architecture. Only ARM, ARM64, x86, and x86_64 are supported.
#endif

static AElf(Sym) apkenv_libdl_symtab[7];
...
construct(void)
{
  const AElf(Sym) symtab[sizeof(apkenv_libdl_symtab) / sizeof(apkenv_libdl_symtab[0])] = {
    {
      .st_name = sizeof(ANDROID_LIBDL_STRTAB) - 1,
    }, {
      .st_name = 0,
      .st_value = (AElf(Addr))(uintptr_t)bionic_dlopen,
      .st_info = STB_GLOBAL << 4,
      .st_shndx = 1,
    }, {
      .st_name = 7,
      .st_value = (AElf(Addr))(uintptr_t)bionic_dlclose,
      .st_info = STB_GLOBAL << 4,
      .st_shndx = 1,
    }, {
      .st_name = 15,
      .st_value = (AElf(Addr))(uintptr_t)bionic_dlsym,
      .st_info = STB_GLOBAL << 4,
      .st_shndx = 1,
    }, {
      .st_name = 21,
      .st_value = (AElf(Addr))(uintptr_t)bionic_dlerror,
      .st_info = STB_GLOBAL << 4,
      .st_shndx = 1,
    }, {
      .st_name = 29,
      .st_value = (AElf(Addr))(uintptr_t)bionic_dladdr,
      .st_info = STB_GLOBAL << 4,
      .st_shndx = 1,
    },
    ...
    {
      .st_name = 36,
      .st_value = (AElf(Addr))(uintptr_t)bionic_dl_unwind_find_exidx,
      .st_info = STB_GLOBAL << 4,
      .st_shndx = 1,
    },
#elif defined(ANDROID_X86_LINKER) || defined(ANDROID_X86_64_LINKER) || defined(ANDROID_AARCH64_LINKER)
    {
      .st_name = 36,
      .st_value = (AElf(Addr))(uintptr_t)bionic_dl_iterate_phdr,
      .st_info = STB_GLOBAL << 4,
      .st_shndx = 1,
    },
#endif
